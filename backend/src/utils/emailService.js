...
const sendEmail = async (options) => {
  ...
  const mail = {
    from: "mail.WordMantra@example.com",
    to: options.email,
...
class EmailService {
  async sendVerificationEmail(email, username, verificationToken) {
    ...
    await sendEmail({
      email,
      subject: "🕉️ Verify Your WordMantra Account",
  ...
  async sendPasswordResetEmail(email, username, resetToken) {
    ...
    await sendEmail({
      email,
      subject: "🔐 Reset Your WordMantra Password",
      mailgenContent: forgotPasswordMailgenContent(username, resetUrl),
    });
  }
}
